...
To switch between WildFly versions, edit the image in `compose.yaml`:
...
├── Makefile                      # WAR build targets
├── BUILD.md                      # Detailed build guide
├── opentelemetry-javaagent.jar   # OTel agent (download manually, see below)
├── wars/                         # Compiled WARs (generated by make build or downloaded)
│   ├── backend.war
│   └── frontend.war
├── java_apps/
│   ├── backend/                  # BackendServlet — reads from PostgreSQL
│   └── frontend/                 # FrontendServlet — calls the backend
└── configs/
